...
/**
 * @typedef {import('@/types').Disease} Disease
 */

export default function DiseasesPage() {
  const navigate = useNavigate();
  const { diseases, loading, error, fetchDiseases } = useSystemStore();
  ...
  const categories = diseases
    ? ['all', ...new Set(diseases.map((d) => d.category).filter(Boolean))]
    : ['all'];
  ...
  const handleAssessRisk = (disease) => {
